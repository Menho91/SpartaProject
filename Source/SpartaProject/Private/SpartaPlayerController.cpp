...
void ASpartaPlayerController::ShowMainMenu(bool bIsRestart)
{
	...
	if (MainMenuWidgetClass)
	{
		...
		if (UTextBlock* StartText = Cast<UTextBlock>(MainMenuWidgetInstance->GetWidgetFromName(TEXT("StartButtonText"))))
		{
			if (bIsRestart)
			{
				StartText->SetText(FText::FromString(FString("Restart")));
			}
			else
			{
				StartText->SetText(FText::FromString(FString("Start")));
			}
		...
		if (bIsRestart)
		{
			...
			UFunction* PlayMenuAnimFunc = MainMenuWidgetInstance->FindFunction(FName("PlayMenuAnim"));
			if (PlayMenuAnimFunc)
			{
